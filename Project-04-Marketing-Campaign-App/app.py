...
def getLLMResponse(query, age_option, tasktype_option, word_limit):
    llm = OpenAI(temperature=0.7, model="gpt-3.5-turbo-instruct")

    example_sets = get_example_sets()
    examples = example_sets.get(age_option, example_sets["Kid"])
    ...
    prefix = f"""You are a {{template_ageoption}}, and {{template_tasktype_option}}:
    ...
    new_prompt_template = FewShotPromptTemplate(
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["template_userInput","template_ageoption","template_tasktype_option","word_limit"],
        example_separator="\n"
    )

    # Convert word_limit to a string here to avoid TypeError in " ".join(...)
    filled_prompt = new_prompt_template.format(
